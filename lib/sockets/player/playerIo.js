...
module.exports.move = function(user){
  if(!Players[user.username]){ return false; }
...
module.exports.killPlayer = function(player){
  if(Players[player]){ Players[player].dead = true; }
};

module.exports.next_level = function(username, dead){
  var level = dead ? Level : Players[username].level + 1;
  Level = level;
