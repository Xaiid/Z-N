...
module.exports.createPlayer = function(options, cb){
  ...
  Players[username] = {
    ...
    //ASK GRID
    x: Coords.level1['pos'+position].x,
    y: Coords.level1['pos'+position].y,
...
module.exports.next_level = function(username){
  ...
  this.socket.emit('Next Level', {username: username, level: level});
  this.socket.broadcast.to(this.socket.room).emit('Next Level', {username: username, level: level}); 
};
